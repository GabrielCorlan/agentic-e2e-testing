...
- **`.github/workflows/tests.yml`** runs the suite on push/PR to `main`: install deps + Playwright browsers, `npm test`, upload the HTML report as an artifact. Needs `BASE_URL`/`TEST_USER`/`TEST_PASS` as GitHub Actions repository secrets (same values as `.env`).
