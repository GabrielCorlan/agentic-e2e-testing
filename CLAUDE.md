...
| `src/objects/elements/` | One class per single HTML tag (`Alert`, `Button`, `Input`, `Link`, ...) |
| `src/objects/components/` | One class per larger reusable composite component built from multiple elements (`AppHeader`, ...) |
| `src/steps/elements/` | Step definitions matching `src/objects/elements/` |
| `src/steps/components/` | Step definitions matching `src/objects/components/` |
...
- **`src/objects/`** + **`src/steps/`** — an object-model split (not a classic Page Object Model), each further split into an `elements/` and a `components/` subfolder that mirror each other one-to-one, one file per HTML tag/component, sharing a name: `src/objects/elements/button.ts` holds a `Button` class wrapping actions (`.click()`, ...) on a `Locator`; `src/steps/elements/button.steps.ts` registers the `When`/`Then` step(s) that use it via `createBdd()` from `playwright-bdd` (not `@cucumber/cucumber` directly — this keeps steps on Playwright's own runner/fixtures rather than Cucumber's). Only create a step/object file pair when a scenario actually needs it; a step that isn't tied to one specific element type (e.g. a plain navigation `Given`) can live wherever it's most related, it doesn't have to force-fit the pairing.
- **`elements/` vs. `components/`** — the deciding question is whether the class wraps exactly one HTML tag or composes several:
  ...
  - A class that starts as an `elements/` wrapper and later grows to compose other elements belongs in `components/` — move it rather than letting a "component" accumulate inside `elements/`.
...
- Generic element steps take the `data-test` id as a Cucumber Expression parameter instead of hardcoding it, so a scenario names the target directly: `When('I click the {string} button', ...)` matches `And I click the "login-submit" button` (see `src/steps/elements/button.steps.ts`). This doesn't apply to steps wrapping a composite/domain action with a fixed target (e.g. `AppHeader`'s sign-in navigation).
